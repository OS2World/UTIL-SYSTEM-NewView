...
Begin
  CreateBitmap;
{  FBitmap.CreateNew( Width, Height, 1 shl 24 );

  for i := 0 to height - 1 do
  begin
    Fbitmap.Canvas.Pen.Color := ValuesToRGB( i * 255 div height, 255 - ( i * 255 div height ) , 0 );
    Fbitmap.Canvas.Line( 0, i, 99, i );
  end;}
  Inherited SetupShow;
End;
...
Begin
  ...
  begin
    // erase
    Canvas.Line( FOldCursorX-FOldCursorSize-1, FOldCursorY,
                 FOldCursorX+FOldCursorSize, FOldCursorY );
    Canvas.Line( FOldCursorX, FOldCursorY-FOldCursorSize-1,
                 FOldCursorX, FOldCursorY+FOldCursorSize );
  end;

  // draw cursor
  Canvas.Line( X-FCursorSize-1, Y,
               X+FCursorSize, Y );
  Canvas.Line( X, Y-FCursorSize-1,
               X, Y+FCursorSize );
  FOldCursorX:= X;
  FOldCursorY:= Y;
...
Procedure TColorWheel.CreateBitmap;
var
 x,y : longint;
 Hue: longint;
 saturation: real;
...
Begin
  ...
  r.Top := Height - 1;
  r.Right := Width - 1;
  FBitmap.Canvas.FillRect( r, Color );
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        C := HSVToRGB( Hue, Saturation, 1.0 );
        // draw the pixel
          C := GpiQueryNearestColor( FBitmap.Canvas.Handle,
                                     0,
                                     C );

        FBitmap.Canvas.Pixels[ X+FMarginWidth, Y+FMarginWidth ]:= C;
      end;
    end;
  end;
End;
...
Begin
  ...
  begin
    ...
    FValueBar.Notification( self, opRemove );
  end;
...
Procedure TColorWheel.Notification( AComponent:TComponent;
...
Begin
  ...
  r.left:= 0; //FMarginWidth;
  r.right:= Width - 1 ; // - FMarginWidth;
