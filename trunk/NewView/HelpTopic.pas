...
Type
  ...
  protected
    ...
    function IPFEscapeCodeSpace( Var State: TParseState;
  ...
  public
    ...
    function SearchForData( Data: pbyte;
...
// and add the end tag.
procedure CheckForAutoURL( Text: TAString;
...
begin
  ...
  if not IsURL( State.TextBlock ) then
  begin
    // not a URL we know
    State.TextBlock.Clear;
    exit;
  end;
  ...
  Text.Insert( State.StartOfTextBlock, T );

  T.Destroy;

  Text.AddString( GetEndLinkTags( State ) );
  ...
  begin
    try
      ...
      begin
        ...
        // Read header
        if not MyRead( _FileHandle,
                       Addr( SlotHeader ),
                       sizeof( SlotHeader ) ) then
...
begin
  ...
  State.StartOfTextBlock := -1;
  State.TextBlock := TAString.Create;

  State.FootnoteLink := nil;

  Text.AddString( '<leftmargin 1>' );
  ...
  begin
    ...
    begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if HighlightMatches <> nil then
                  HighlightMatches.Add( pointer( Text.Length ) );
      ...
      begin
        ...
        if ShowCodes then
        begin
          Text.AddString( '[' + IntToHex( LocalDictIndex, 2 ) );
